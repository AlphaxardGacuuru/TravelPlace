...
<form action="{{ url('admin/blog/update/'.$blog->id) }}"
	  method="post"
	...
	<div class="card shadow mb-4">
		...
		<div class="card-body">
			...
			<div class="form-group">
				...
				<textarea name="blog_content_short"
						  class="form-control h_100"
						  cols="30"
						  rows="10">{{ $blog->blog_content_short }}</textarea>
			</div>
			<div class="form-group">
				<label for="">Existing Blog Photo</label>
				<div>
					<img src="{{ asset('public/uploads/'.$blog->blog_photo) }}"
						 alt=""
						 class="w_200">
				</div>
			</div>
				...
				<div class="col-md-4">
					<div class="form-group">
						...
						<select name="category_id"
							...
							<option value="{{ $row->id }}"
									@if($row->id == $blog->category_id) selected @endif>{{ $row->category_name }}
							</option>
							@endforeach
						</select>
					</div>
			...
			<div class="row">
				<div class="col-md-4">
					<div class="form-group">
						...
						<select name="comment_show"
								class="form-control">
							<option value="Yes"
									@if($blog->comment_show == 'Yes') selected @endif>Yes</option>
							<option value="No"
									@if($blog->comment_show == 'No') selected @endif>No</option>
						</select>
					</div>
				</div>
			</div>
		...
		<div class="card-body">
			...
			<div class="form-group">
				...
				<textarea name="seo_meta_description"
						  class="form-control h_100"
						  cols="30"
						  rows="10">{{ $blog->seo_meta_description }}</textarea>
			</div>
			@can("update-blog-section")
			<button type="submit"
